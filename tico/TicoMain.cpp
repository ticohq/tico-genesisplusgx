/// @file TicoMain.cpp
/// @brief Entry point for tico-integrated NRO
...
static std::string g_consoleSlug = "genesis"; // Default slug, overridden by argv
...
//==============================================================================
// (SimpleGameRenderer removed - always tico/overlay mode)
...
bool InitWindow()
{
    ...
    g_window = SDL_CreateWindow("Genesis-Plus-GX",
...
int main(int argc, char *argv[])
{
    ...
    // Parse args: argv[1] = console slug, argv[2+] = ROM path
    // Always tico mode for Genesis Plus GX
    std::string romPath;
    if (argc >= 3) {
        g_consoleSlug = argv[1];
        romPath = argv[2];
        LOG_INFO("HOME", "Console slug: %s", g_consoleSlug.c_str());
        LOG_INFO("HOME", "ROM path: %s", romPath.c_str());
    } else if (argc >= 2) {
        // Fallback: single arg is ROM path, default slug
        romPath = argv[1];
        LOG_INFO("HOME", "Single arg mode, ROM path: %s", romPath.c_str());
    } else {
        LOG_WARN("HOME", "No arguments provided, using defaults");
    }

    // Configure per-system paths based on slug
    TicoConfig::SetSlug(g_consoleSlug);
    LOG_INFO("HOME", "Configured paths for slug '%s': saves=%s states=%s",
             g_consoleSlug.c_str(),
             TicoConfig::SAVES_PATH.c_str(),
             TicoConfig::STATES_PATH.c_str());
    ...
    LOG_INFO("HOME", "Genesis Plus GX starting (slug: %s)...", g_consoleSlug.c_str());
    ...
    // Ensure save/state dirs exist
    {
        ...
        if (stat(TicoConfig::STATES_PATH.c_str(), &st) == -1) {
            mkdir("sdmc:/tico/states", 0777);
            mkdir(TicoConfig::STATES_PATH.c_str(), 0777);
        }
    }

    if (romPath.empty()) {
        LOG_WARN("HOME", "No ROM path provided.");
    }
