...
static std::string GetStatePath(TicoCore *core, int slot)
{
    ...
    return TicoConfig::STATES_PATH + romName + ".state" + std::to_string(slot);
}
...
void TicoOverlay::RenderGame(ImDrawList *dl, ImVec2 displaySize, unsigned int texture,
    ...
    if (texture == 0) return;
    float baseW = (width > 0) ? (float)width : 320.0f;
    float baseH = (height > 0) ? (float)height : 240.0f;
    ...
    if (m_displayMode == TicoDisplayMode::Integer) {
        ...
        case TicoDisplaySize::Stretch: dstWidth = displaySize.x; dstHeight = displaySize.y; break;
...
void TicoOverlay::RenderSettingsMenu(ImDrawList *dl, ImVec2 displaySize) {
    ...
    for (int i = 0; i < N; i++) {
        ...
        if (i == 0) { label = tr("emulator_display_mode"); value = (m_displayMode == TicoDisplayMode::Integer) ? tr("emulator_integer") : tr("emulator_display"); }
        else if (i == 1) {
            label = tr("emulator_size");
            if (m_displayMode == TicoDisplayMode::Integer) {
                switch (m_displaySize) { case TicoDisplaySize::_1x: value="1x"; break; case TicoDisplaySize::_2x: value="2x"; break; default: value=tr("emulator_auto"); break; }
            } else {
                switch (m_displaySize) { case TicoDisplaySize::Stretch: value=tr("emulator_stretch"); break; case TicoDisplaySize::_4_3: value="4:3"; break; case TicoDisplaySize::_16_9: value="16:9"; break; default: value=tr("emulator_original"); break; }
            }
        } else if (i == 2) {
...
bool TicoOverlay::HandleInput(SDL_GameController *controller) {
    ...
    if (dirChanged && m_currentMenu == OverlayMenu::Settings) {
        if (m_settingsSelection == 0) {
            m_displayMode = (m_displayMode == TicoDisplayMode::Display) ? TicoDisplayMode::Integer : TicoDisplayMode::Display;
            m_displaySize = (m_displayMode == TicoDisplayMode::Integer) ? TicoDisplaySize::Auto : TicoDisplaySize::_4_3;
    ...
    if (confirm && !m_confirmHeld && debounced) {
        ...
        if (m_currentMenu == OverlayMenu::QuickMenu) {
            ...
            if (m_settingsSelection == 0) {
                ...
                if (m_displayMode == TicoDisplayMode::Integer) { int s = (int)m_displaySize; s = (s >= 6) ? 4 : s+1; m_displaySize = (TicoDisplaySize)s; }
                else { int s = (int)m_displaySize; s = (s >= 3) ? 0 : s+1; m_displaySize = (TicoDisplaySize)s; }
                ApplyScalingSettings(true);
            } else if (m_settingsSelection == 2) {
...
#ifdef __SWITCH__
    std::string configPath = "sdmc:/tico/config/cores/genesis_plus_gx.jsonc";
#else
    std::string configPath = "tico/config/cores/genesis_plus_gx.jsonc";
#endif
    ...
    if (file.is_open()) {
        ...
        if (!j.is_discarded()) {
            if (j.contains("display_mode") && j["display_mode"].is_string()) {
                m_displayMode = (j["display_mode"].get<std::string>() == "Integer") ? TicoDisplayMode::Integer : TicoDisplayMode::Display;
            } else m_displayMode = TicoDisplayMode::Integer;
            if (j.contains("display_size") && j["display_size"].is_string()) {
                std::string v = j["display_size"].get<std::string>();
                if (v=="Stretch") m_displaySize = TicoDisplaySize::Stretch;
                else if (v=="16:9") m_displaySize = TicoDisplaySize::_16_9;
                else if (v=="Original") m_displaySize = TicoDisplaySize::Original;
                else if (v=="1x") m_displaySize = TicoDisplaySize::_1x;
                else if (v=="2x") m_displaySize = TicoDisplaySize::_2x;
                else if (v=="Auto") m_displaySize = TicoDisplaySize::Auto;
                else m_displaySize = TicoDisplaySize::_4_3;
            } else m_displaySize = TicoDisplaySize::Auto;
        ...
        } else { m_displayMode = TicoDisplayMode::Integer; m_displaySize = TicoDisplaySize::Auto; }
    } else { m_displayMode = TicoDisplayMode::Integer; m_displaySize = TicoDisplaySize::Auto; }
...
void TicoOverlay::SaveCoreSettings() {
    ...
    switch (m_displaySize) {
    case TicoDisplaySize::Stretch: sizeStr="Stretch"; break; case TicoDisplaySize::_4_3: sizeStr="4:3"; break;
    case TicoDisplaySize::_16_9: sizeStr="16:9"; break; case TicoDisplaySize::Original: sizeStr="Original"; break;
    case TicoDisplaySize::_1x: sizeStr="1x"; break; case TicoDisplaySize::_2x: sizeStr="2x"; break;
    case TicoDisplaySize::Auto: sizeStr="Auto"; break; default: break;
    }
