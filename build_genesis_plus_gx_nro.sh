...
echo "=== Building Genesis Plus GX NRO with Tico Overlay ==="
...
# ============================================================
# Step 1: Build Genesis Plus GX as a static library (.a)
# ============================================================
echo "--- Step 1: Building Genesis Plus GX static library ---"

cd "$ROOT_DIR"
make -f Makefile.libretro clean platform=libnx 2>/dev/null || true
make -f Makefile.libretro -j$(nproc) platform=libnx

STATIC_LIB="$ROOT_DIR/genesis_plus_gx_libretro_libnx.a"
...
# ============================================================
echo "--- Step 3: Linking genesis_plus_gx_tico.elf ---"

ELF_OUTPUT="$BUILD_DIR/genesis_plus_gx_tico.elf"

LINK_FLAGS="-specs=$LIBNX/switch.specs -march=armv8-a+crc+crypto -mtune=cortex-a57 -mtp=soft -fPIE"
LINK_FLAGS="$LINK_FLAGS -Wl,--gc-sections -Wl,--allow-multiple-definition -Wl,-Map=$BUILD_DIR/genesis_plus_gx_tico.map"
...
NRO_OUTPUT="$BUILD_DIR/tico-genesis_plus_gx.nro"
ELF2NRO="$DEVKITPRO/tools/bin/elf2nro"
NACPTOOL="$DEVKITPRO/tools/bin/nacptool"

# Create NACP
NACP_FILE="$BUILD_DIR/genesis_plus_gx.nacp"
$NACPTOOL --create "tico Genesis Plus GX" "ticoverse.com" "1.0.0" "$NACP_FILE"
...
if [ -f "$NRO_OUTPUT" ]; then
    ...
    echo "Deploy to: sdmc:/tico/cores/tico-genesis_plus_gx.nro"
    echo "======================================"
else
    echo "Error: tico-genesis_plus_gx.nro not found"
    exit 1
fi
